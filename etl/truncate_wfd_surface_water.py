...
def truncate_wfd_surface_water():
    """
    Empties all rows from the 'wfd_surface_water' table without destroying the schema.

    Use this before re-loading WFD data (e.g. when an updated INSPIRE export is
    available from https://www.pdok.nl/introductie/-/article/krw-oppervlaktewaterlichamen-inspire-geharmoniseerd-)
    ...
    try:
        with engine.begin() as conn:
            ...
            if exists:
                ...
                print(f"⚠️  Table '{table_name}' does not exist yet. Run load_wfd_surface_water.py first.")
