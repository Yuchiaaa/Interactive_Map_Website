...
def load_woondeals_gdal(file_path):
    """
    Industrial-grade GDAL (ogr2ogr) pipeline for Regional Housing Agreements (Woondeals).
    ...
    try:
        ...
        with engine.begin() as conn:
            try:
                ...
                conn.execute(text("ALTER TABLE woondeals ALTER COLUMN geom TYPE geometry(MultiPolygon, 4326) USING ST_Multi(geom);"))
                
                # Ensure the 'id' column auto-increments perfectly
                conn.execute(text("CREATE SEQUENCE IF NOT EXISTS woondeals_id_seq;"))
                conn.execute(text("ALTER TABLE woondeals ALTER COLUMN id SET DEFAULT nextval('woondeals_id_seq');"))
            except Exception:
        ...
        cmd = [
            ...
            OGR_PG_CONN_STRING,
            file_path,
            "-nln", "woondeals",         # Target PostGIS table for Woondeals
        ...
        print("Success: Woondeals securely imported into the database.")
...
if __name__ == "__main__":
    ...
    woondeals_file = "/Users/yuchia/Downloads/RegionaleWoondeals.gpkg" 
    
    # load_woondeals_gdal(woondeals_file)
    print("Woondeals Script ready. Uncomment the execution lines to run.")