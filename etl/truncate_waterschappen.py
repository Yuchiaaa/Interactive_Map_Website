...
def truncate_waterschappen():
    """
    Empties all rows from the 'waterschappen' table without destroying the schema.

    Use this before re-loading Waterschappen data (e.g. when an updated boundary
    export is available from https://www.pdok.nl/introductie/-/article/waterschappen-waterschapsgrenzen-imso)
    ...
    try:
        with engine.begin() as conn:
            ...
            if exists:
                ...
                print(f"⚠️  Table '{table_name}' does not exist yet. Run load_waterschappen.py first.")
